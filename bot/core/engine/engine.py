...
class SignalEngine:
    ...
    def __init__(
        ...
        configured_concurrency = getattr(
            self._settings.runtime, "analysis_concurrency", 10
        )
        self._semaphore = asyncio.Semaphore(max(1, int(configured_concurrency)))
    ...
    def get_engine_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "timeout_seconds": self._timeout,
            "semaphore_limit": int(
                getattr(self._settings.runtime, "analysis_concurrency", 10)
            ),
        }
