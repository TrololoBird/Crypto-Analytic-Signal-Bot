...
def detect_price_velocity(
    ...
    *,
    ...
    if min(close, atr) <= 0.0 or "close" not in work.columns:
        return None
    returns = work.select(pl.col("close").pct_change().alias("ret"))["ret"].drop_nulls()
    if returns.len() < lookback + 5:
        return None
    recent_ret = as_float(returns.tail(1).item(), 0.0)
    ret_std = as_float(returns.tail(lookback + 20).std(), 0.0)
    if ret_std <= 0.0:
        return None
    velocity_z = recent_ret / ret_std
    if abs(velocity_z) <= threshold:
        return None
    direction = "long" if velocity_z > 0.0 else "short"
    entry = as_float(work.item(-1, "open") if direction == "long" else work.item(-1, "open"))
    return SpecHit(
        ...
        atr=atr,
        timeframe=timeframe,
        reasons=(f"velocity_z={velocity_z:.2f}",),
        structure_clarity=min(1.0, abs(velocity_z) / 3.0),
