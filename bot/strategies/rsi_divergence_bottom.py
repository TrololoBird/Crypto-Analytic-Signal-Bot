    ...
    *,
    ...
    if direction is None and current_atr > 0.0 and current_close > 0.0:
        ...
        bullish_reversal_bar = (
            current_close >= current_open or close_position >= long_close_position
        )
        bearish_reversal_bar = (
            current_close <= current_open or close_position <= short_close_position
        )
        if (
            recent_low_near_previous
            and (recent_rsi_low >= prev_rsi_low + rsi_delta * 0.5 or low_rsi_recovered)
            ...
            and (recent_rsi_high <= prev_rsi_high - rsi_delta * 0.5 or high_rsi_rejected)
    ...
    if direction is None and current_atr > 0.0:
        ...
        if lows and highs and rsis:
            ...
            if (
                ...
                direction = "long"
                entry_anchor = max(
                    lows[low_idx] + current_atr * 0.30, current_close - current_atr * 0.20
                )
                ...
                direction = "short"
                entry_anchor = min(
                    highs[high_idx] - current_atr * 0.30, current_close + current_atr * 0.20
                )
                score_penalty = float(params.get("rsi_recovery_penalty", 0.86))
