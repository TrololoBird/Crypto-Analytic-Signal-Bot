...
def detect_volume_anomaly(frame: pl.DataFrame, *, timeframe: str = "15m") -> SpecHit | None:
    ...
    return SpecHit(
        ...
        timeframe=timeframe,
        reasons=(
            f"volume_spike={row['volume'] / volume_mean:.2f}x",
            f"body_ratio={row['spec_body_ratio']:.2f}",
        ),
...
def detect_volume_anomaly_setup(
    prepared: PreparedSymbol,
    ...
    return run_setup_detection(
        ...
        defaults=defaults,
        effective=effective,
        spec_detect=detect_volume_anomaly,
        extended_detect=_detect_volume_anomaly_extended,
...
class VolumeAnomalySetup(SpecDetectorSetup):
    ...
    detect_setup = detect_volume_anomaly_setup


__all__ = ["VolumeAnomalySetup"]
