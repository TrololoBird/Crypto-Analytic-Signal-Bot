...
def stochastic(df: pl.DataFrame, period: int = 14, smooth_k: int = 3, smooth_d: int = 3) -> tuple[pl.Series, pl.Series]:
...
def cci(df: pl.DataFrame, period: int = 20, *, plta: Any = None, has_talib: bool = False) -> pl.Series:
    ...
    if has_talib:
        return materialize_series(cast(Any, plta).CCI(pl.col("high"), pl.col("low"), pl.col("close"), timeperiod=float(period)), df=df, name=f"cci{period}")
...
def mfi(df: pl.DataFrame, period: int = 14, *, plta: Any = None, has_talib: bool = False) -> pl.Series:
    ...
    if has_talib:
        return materialize_series(cast(Any, plta).MFI(pl.col("high"), pl.col("low"), pl.col("close"), pl.col("volume"), timeperiod=float(period)), df=df, name=f"mfi{period}")
    typical = (df["high"] + df["low"] + df["close"]) / 3.0
    money_flow = typical * df["volume"]
    delta = typical.diff()
    pos = pl.Series("pos", [float(mf or 0.0) if float(chg or 0.0) > 0.0 else 0.0 for mf, chg in zip(money_flow, delta, strict=False)], dtype=pl.Float64)
    neg = pl.Series("neg", [float(mf or 0.0) if float(chg or 0.0) < 0.0 else 0.0 for mf, chg in zip(money_flow, delta, strict=False)], dtype=pl.Float64)
    vals: list[float] = []
    for p, n in zip(pos.rolling_sum(window_size=period), neg.rolling_sum(window_size=period), strict=False):
        pv = float(p or 0.0)
        nv = float(n or 0.0)
...
def cmf(df: pl.DataFrame, period: int = 20) -> pl.Series:
    ...
    for high, low, close in zip(df["high"], df["low"], df["close"], strict=False):
        ...
        width = h - l
        multipliers.append(0.0 if width <= 0.0 else ((c - l) - (h - c)) / width)
    mfv = pl.Series("mfm", multipliers, dtype=pl.Float64) * df["volume"]
    return ((mfv.rolling_sum(window_size=period) / df["volume"].rolling_sum(window_size=period)).fill_nan(0.0).rename(f"cmf{period}"))
...
def ultimate_oscillator(df: pl.DataFrame, p1: int = 7, p2: int = 14, p3: int = 28) -> pl.Series:
    ...
    max_high = materialize_series(pl.max_horizontal(df["high"], prev_close), df=df, name="uo_max_high")
    bp = df["close"] - min_low
    tr = max_high - min_low
    ...
    return clean_non_finite((100.0 * ((4.0 * avg1) + (2.0 * avg2) + avg3) / 7.0).rename("uo"), fill=50.0)


def add_oscillator_features(df: pl.DataFrame, *, plta: Any = None, has_talib: bool = False) -> pl.DataFrame:
