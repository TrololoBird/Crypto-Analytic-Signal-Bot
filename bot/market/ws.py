...
class MessageBuffer:
    ...
    def _drop_oldest_batch(self) -> int:
        ...
        if not batch:
            return 0
        closed_kline_indexes = [idx for idx, msg in enumerate(batch) if self._message_priority(msg) == 100]
        non_closed_indexes = [idx for idx, msg in enumerate(batch) if self._message_priority(msg) < 100]
        if closed_kline_indexes and non_closed_indexes:
            LOG.debug('message buffer backpressure with protected closed klines | protected=%d queued=%d', len(closed_kline_indexes), len(batch))
        ...
        if non_closed_indexes:
            ...
            drop_indexes = {idx for idx in non_closed_indexes if self._message_priority(batch[idx]) == lowest_priority}
        else:
            drop_index, _ = min(enumerate(batch), key=lambda item: (self._closed_kline_timestamp(item[1]) if self._closed_kline_timestamp(item[1]) is not None else float('inf'), item[0]))
            drop_indexes = {drop_index}
            LOG.warning('message buffer forced to drop a closed kline (all high-priority) | dropped_total=%d', self._dropped_count + 1)
        dropped = 0
    ...
    async def put(self, msg: JsonDict) -> bool:
        ...
        try:
            ...
            if dropped <= 0:
                ...
                LOG.debug('message buffer compact failed | dropped_oldest=%d processed=%d', self._dropped_count, self._processed_count)
                return False
            ...
            if self._dropped_count - self._last_compaction_log_count >= 10000:
                ...
                LOG.debug('message buffer compacted | dropped_oldest=%d processed=%d size=%d', self._dropped_count, self._processed_count, self._buffer.qsize())
            return buffered
        else:
            return True
    ...
    def get_stats(self) -> dict[str, int]:
        ...
        return {'size': self._buffer.qsize(), 'maxsize': self._buffer.maxsize, 'dropped': self._dropped_count, 'processed': self._processed_count, 'protected_kline_drop_count': self._protected_kline_drop_count}
...
def base_streams_for_symbols(manager: Any, symbols: list[str]) -> list[str]:
    return [f'{sym}@kline_{interval}' for sym in _normalized_symbols(symbols) for interval in manager._cfg.kline_intervals]
...
def public_streams_for_symbols(manager: Any, symbols: list[str]) -> list[str]:
    ...
    return [stream for stream in streams if not any((stream.endswith(forbidden) for forbidden in FORBIDDEN_STREAM_SUFFIXES))]
...
def stream_endpoint_class(stream: str) -> str:
    ...
    if '@bookticker' in normalized or '@depth' in normalized:
        return 'public'
    allowed_market = ('@kline_', '@aggtrade', '@markprice', '!markprice@arr@1s', '!markprice@arr', '!ticker@arr', '!miniticker@arr', '!forceorder@arr')
...
def recompute_intended_streams(manager: Any) -> None:
    ...
    LOG.debug('subscription budget | market=%d public=%d depth=%d agg=%d limit=%d', budget_plan.total_market, budget_plan.total_public, budget_plan.depth_streams, budget_plan.agg_trade_streams, budget_plan.budget_limit)

def validate_endpoint_stream_limits(manager: Any) -> None:
    max_streams = int(getattr(manager, '_max_streams_per_connection', DEFAULT_MAX_STREAMS_PER_CONNECTION))
...
async def send_subscription_command(manager: Any, endpoint: str, method: str, streams: list[str]) -> None:
    if not streams:
        return
    ...
    for offset in range(0, len(streams), chunk_size):
        ...
        try:
            ...
            LOG.debug('ws %s chunk | endpoint=%s offset=%d streams=%d', method, endpoint, offset, len(chunk))
...
async def resubscribe_all(manager: Any, endpoint: str, ws: Any) -> None:
    ...
    if not streams:
        return
    if len(streams) > 200:
        LOG.info('ws high stream count | endpoint=%s streams=%d symbols=%d - consider reducing shortlist_limit in config', endpoint, len(streams), len(manager._symbols))
    ...
    LOG.info('ws resubscribe sent | endpoint=%s streams=%d chunks=%d', endpoint, len(streams), chunk_count)
...
def get_stats(manager: Any) -> dict[str, Any]:
    stats: dict[str, Any] = {'streams_total': len(manager._intended_streams), 'streams_active': len(manager._stream_last_message_ts), 'slow_streams': list(manager._slow_streams), 'slow_streams_count': len(manager._slow_streams), 'buffer_stats': manager._message_buffer.get_stats()}
...
def get_global_ticker_data(manager: Any) -> list[JsonDict]:
    ...
    for symbol, ticker in manager._ticker_cache.items():
        ...
        if now - last_update > manager._cfg.market_ticker_freshness_seconds:
            continue
        result.append({'symbol': symbol, 'quote_volume': ticker.get('quote_volume', 0.0), 'price_change_percent': ticker.get('price_change_percent', 0.0), 'last_price': ticker.get('last_price', 0.0), 'trade_count': int(float(ticker.get('trade_count') or 0))})
    return result
...
def get_depth_imbalance(manager: Any, symbol: str) -> float | None:
    ...
    return depth_imbalance_from_book(bid_qty=bid_qty, ask_qty=ask_qty, delta_ratio=None if snapshot is None else snapshot.delta_ratio)
...
def get_microprice_bias(manager: Any, symbol: str) -> float | None:
    ...
    bid_qty, ask_qty = manager._book_qty.get(symbol, (None, None))
    snapshot = manager.get_agg_trade_snapshot(symbol)
    return microprice_bias_from_book(bid=bid, ask=ask, bid_qty=bid_qty, ask_qty=ask_qty, delta_ratio=None if snapshot is None else snapshot.delta_ratio)
...
def get_liquidation_rollups(manager: Any, symbol: str | None=None, window_seconds: int=900) -> dict[str, float] | None:
    ...
    if total <= 0.0:
        return None
    return {'liquidation_long_notional': long_notional, 'liquidation_short_notional': short_notional, 'liquidation_total_notional': total, 'liquidation_score': (short_notional - long_notional) / total}

def get_liquidation_sentiment(manager: Any, symbol: str | None=None, window_seconds: int=60) -> float | None:
...
def get_liquidation_age_seconds(manager: Any, symbol: str | None=None, window_seconds: int=60) -> float | None:
    cutoff_ms = int(time.time() * 1000) - window_seconds * 1000
...
def should_throttle_ticker_update(manager: Any, symbol: str) -> bool:
    ...
    if elapsed_ms < manager._min_ticker_update_interval_ms:
        ...
        if now - last_logged >= 30.0:
            ...
            LOG.debug('ticker throttled | symbol=%s elapsed=%.0fms min=%.0fms', symbol, elapsed_ms, manager._min_ticker_update_interval_ms)
        return True
...
class FuturesWSManager:
    ...
    def __init__(self, rest_client: BinanceFuturesMarketData, config: WSConfig, *, proxy_url: str | None=None, trust_env: bool=True) -> None:
        ...
        self._force_order_buffer: collections.deque[tuple[int, str, str, float, float]] = collections.deque(maxlen=500)
        self._stream_task: asyncio.Task[None] | None = None
        self._stream_tasks: dict[str, asyncio.Task[None] | None] = dict.fromkeys(_WS_ENDPOINTS)
        self._running = False
        self._connected = asyncio.Event()
        self._connected_endpoints: dict[str, asyncio.Event] = {endpoint: asyncio.Event() for endpoint in _WS_ENDPOINTS}
        ...
        self._intended_streams_by_endpoint: dict[str, set[str]] = {endpoint: set() for endpoint in _WS_ENDPOINTS}
        ...
        self._last_reconnect_reason_by_endpoint: dict[str, str] = dict.fromkeys(_WS_ENDPOINTS, 'not_started')
    ...
    def _schedule_backfill(self, symbols: list[str], *, name: str) -> asyncio.Task[None] | None:
        scheduled_symbols = [symbol for symbol in self._normalize_symbol_list(symbols) if symbol not in self._backfill_symbols_inflight]
    ...
    async def start(self, symbols: list[str]) -> None:
        ...
        for endpoint in self._active_endpoint_classes():
            self._stream_tasks[endpoint] = asyncio.create_task(self._run_stream(endpoint), name=f'ws_manager_stream:{endpoint}')
        self._stream_task = self._stream_tasks.get(_WS_MARKET) or self._stream_tasks.get(_WS_PUBLIC)
        self._buffer_processor_task = asyncio.create_task(self._process_buffered_messages(), name='ws_buffer_processor')
        LOG.info('ws_manager started | symbols=%d endpoints=%s', len(self._symbols), list(self._active_endpoint_classes()))
        if self._symbols:
    ...
    async def stop(self) -> None:
        ...
        tasks_to_cancel = [task for task in self._stream_tasks.values() if task is not None and (not task.done())]
    ...
    def _active_endpoint_classes(self) -> tuple[str, ...]:
        return tuple((endpoint for endpoint in _WS_ENDPOINTS if self._intended_streams_by_endpoint.get(endpoint)))
    ...
    def state_snapshot(self) -> dict[str, Any]:
        ...
        fresh_tickers = sum((1 for sym, ts in self._ticker_update_times.items() if now - ts <= self._cfg.market_ticker_freshness_seconds))
        freshness = self._cfg.market_ticker_freshness_seconds
        fresh_mark_prices = sum((1 for sym, ts in self._mark_price_update_times.items() if sym and now - ts <= freshness))
        if fresh_mark_prices <= 0:
            fresh_mark_prices = sum((1 for sym, row in self._mark_price_cache.items() if sym and float(row.get('mark_price') or 0.0) > 0.0 and (now - float(row.get('updated_at') or 0.0) <= freshness)))
        fresh_book_tickers = sum((1 for sym, ts in self._book_update_times.items() if sym in self._symbols and now - ts <= self._cfg.market_ticker_freshness_seconds))
        fresh_depth_books = sum((1 for sym, ts in self._depth_update_times.items() if sym in self._symbols and now - ts <= self._cfg.market_ticker_freshness_seconds))
        fresh_klines_15m = sum((1 for sym in self._symbols if self._is_interval_fresh(sym, '15m')))
        connected_urls = {endpoint: url for endpoint, url in self._connected_urls.items() if url is not None}
        connected_endpoints = [endpoint for endpoint in _WS_ENDPOINTS if self._connected_endpoints[endpoint].is_set()]
        ...
        anchor_symbols_in_agg_trade = sum((1 for anchor in ORDER_FLOW_ANCHOR_SYMBOLS if anchor in agg_symbols))
        return {'active_stream_count': len(self._intended_streams), 'intended_stream_count': len(self._intended_streams), 'warm_symbols': warm, 'total_symbols': total, 'reconnect_count': self._connect_count, 'last_event_lag_ms': self._last_event_lag_ms, 'avg_latency_ms': self._get_current_latency_ms(), 'last_message_age_seconds': self._last_message_age_seconds(), 'public_last_message_age_seconds': self._last_message_age_seconds(_WS_PUBLIC), 'market_last_message_age_seconds': self._last_message_age_seconds(_WS_MARKET), 'ticker_cache_age_seconds': ticker_age, 'buffer_message_count': self._message_buffer.get_stats()['size'], 'message_buffer': self._message_buffer.get_stats(), 'stale_event_drop_count': self._stale_event_drop_count, 'fresh_tickers': fresh_tickers, 'fresh_mark_prices': fresh_mark_prices, 'fresh_book_tickers': fresh_book_tickers, 'fresh_depth_books': fresh_depth_books, 'fresh_klines_15m': fresh_klines_15m, 'public_connect_count': self._connect_counts[_WS_PUBLIC], 'market_connect_count': self._connect_counts[_WS_MARKET], 'mark_price_fresh_symbols': fresh_mark_prices, 'liq_buffer_size': len(self._force_order_buffer), 'stale_kline_stream_count': len(self._stale_kline_streams()), 'connected_ws_url': connected_urls or None, 'ws_endpoint_class': connected_endpoints or None, 'public_subscription_ack_count': self._subscription_ack_count[_WS_PUBLIC], 'market_subscription_ack_count': self._subscription_ack_count[_WS_MARKET], 'public_subscription_error': self._subscription_errors[_WS_PUBLIC], 'market_subscription_error': self._subscription_errors[_WS_MARKET], 'order_flow_tracked_count': len(self._tracked_symbols), 'anchor_symbols_in_agg_trade': anchor_symbols_in_agg_trade, 'last_shortlist_rebuild_age_s': round(now - self._last_shortlist_rebuild_ts, 1) if self._last_shortlist_rebuild_ts > 0 else None}
    ...
    def _get_current_latency_ms(self) -> float | None:
        ...
        all_latencies = [sum(latencies) / len(latencies) for latencies in self._stream_latency_ms.values() if latencies]
        if all_latencies:
    ...
    async def subscribe(self, symbols: list[str]) -> None:
        ...
        async with self._lock:
            ...
            previous_by_endpoint = {endpoint: set(streams) for endpoint, streams in self._intended_streams_by_endpoint.items()}
            self._recompute_intended_streams()
            current_by_endpoint = {endpoint: set(streams) for endpoint, streams in self._intended_streams_by_endpoint.items()}
        if removed_symbols:
        ...
        if self._running:
            ...
            LOG.info('ws_manager subscription update | added=%d removed=%d total=%d endpoints=%s', len(new_symbols), len(removed_symbols), len(requested_symbols), list(self._active_endpoint_classes()))
            if new_symbols:
    ...
    async def set_tracked_symbols(self, symbols: list[str]) -> None:
        ...
        if getattr(self._cfg, 'subscribe_depth', False):
            ...
            if self._running and (public_task is None or public_task.done()) and current_public_streams:
                self._stream_tasks[_WS_PUBLIC] = asyncio.create_task(self._run_stream(_WS_PUBLIC), name='ws_manager_stream:public')
            elif self._ws_conns[_WS_PUBLIC] is not None and self._running:
                await self._send_subscription_command(_WS_PUBLIC, 'UNSUBSCRIBE', list(previous_public_streams - current_public_streams))
                await self._send_subscription_command(_WS_PUBLIC, 'SUBSCRIBE', list(current_public_streams - previous_public_streams))
        ...
        if self._running and (market_task is None or market_task.done()) and current_market_streams:
            self._stream_tasks[_WS_MARKET] = asyncio.create_task(self._run_stream(_WS_MARKET), name='ws_manager_stream:market')
            self._stream_task = self._stream_tasks[_WS_MARKET]
        elif self._ws_conns[_WS_MARKET] is not None and self._running:
            await self._send_subscription_command(_WS_MARKET, 'UNSUBSCRIBE', list(previous_market_streams - current_market_streams))
            await self._send_subscription_command(_WS_MARKET, 'SUBSCRIBE', list(current_market_streams - previous_market_streams))
        if removed:
            async with self._data_lock:
                for symbol in removed:
                    self._agg_trades.pop(symbol, None)
    ...
    def _stale_symbols(self) -> list[str]:
        ...
        return [symbol for symbol in self._symbols if any((not self._is_interval_fresh(symbol, interval) for interval in self._cfg.kline_intervals))]
    ...
    async def _maybe_backfill_after_disconnect(self, *, elapsed: float, stale_symbols: list[str]) -> None:
        ...
        if not self._should_backfill_after_disconnect(elapsed=elapsed, stale_symbols=stale_symbols):
            LOG.info('reconnect backfill skipped | short_disconnect=%.1fs stale_symbols=%d', elapsed, len(stale_symbols))
            return
    ...
    async def get_symbol_frames(self, symbol: str) -> SymbolFrames | None:
        ...
        return SymbolFrames(symbol=symbol, df_1h=interval_dfs.get('1h', pl.DataFrame()), df_15m=interval_dfs.get('15m', pl.DataFrame()), bid_price=bid, ask_price=ask, df_5m=interval_dfs.get('5m', pl.DataFrame()), df_4h=interval_dfs.get('4h', pl.DataFrame()), bid_qty=bid_qty, ask_qty=ask_qty)
    ...
    def get_agg_trade_snapshot(self, symbol: str, *, window_seconds: int | None=None) -> AggTradeSnapshot | None:
        ...
        return AggTradeSnapshot(symbol=symbol, trade_count=count, buy_qty=buy_qty, sell_qty=sell_qty, delta_ratio=delta_ratio)

    def get_depth_book_snapshot(self, symbol: str) -> dict[str, tuple[tuple[float, float], ...]] | None:
    ...
    def get_liquidation_sentiment(self, symbol: str | None=None, window_seconds: int=60) -> float | None:
    ...
    def get_liquidation_rollups(self, symbol: str | None=None, window_seconds: int=900) -> dict[str, float] | None:
    ...
    def get_liquidation_age_seconds(self, symbol: str | None=None, window_seconds: int=60) -> float | None:
    ...
    async def rebuild_shortlist_on_demand(self, symbol_meta: list[Any], settings: Any) -> tuple[list[Any], dict[str, Any]]:
        ...
        async with self._shortlist_rebuild_lock:
            ...
            if time_since_last < self._shortlist_rebuild_interval_seconds:
                LOG.debug('shortlist rebuild throttled | age=%.1fs < interval=%.1fs', time_since_last, self._shortlist_rebuild_interval_seconds)
            ...
            if self.is_ticker_cache_warm():
                ...
                tickers = []
            shortlist, summary = build_shortlist(symbol_meta, tickers, settings, seed_source='ws_light')
    ...
    async def _backfill(self, symbols: list[str]) -> None:
        ...
        for symbol in symbols:
            ...
            if cooldown_until is not None and now < cooldown_until:
                LOG.info('backfill cooldown active | symbol=%s remaining=%.1fs', symbol, cooldown_until - now)
                continue
            filtered_symbols.append(symbol)
        tasks = [self._backfill_one(symbol, interval) for symbol in filtered_symbols for interval in self._cfg.kline_intervals]
    ...
    async def _backfill_one(self, symbol: str, interval: str) -> None:
        try:
            async with self._backfill_sem:
                df = await self._rest.fetch_klines(symbol, interval, limit=self._cfg.kline_cache_size)
            if df is None or df.is_empty():
                LOG.debug('backfill_empty_frame', extra={'symbol': symbol, 'interval': interval})
                return
            rows = df.to_dicts()
            deq: collections.deque[JsonDict] = collections.deque(rows, maxlen=self._cfg.kline_cache_size)
            async with self._data_lock:
            ...
            if self._cfg.backfill_failure_cooldown_seconds > 0:
                self._backfill_cooldowns[symbol] = time.monotonic() + self._cfg.backfill_failure_cooldown_seconds
    ...
    async def _run_stream(self, endpoint: str) -> None:
        await run_endpoint_loop(self, endpoint=endpoint, backoff_reset_after_seconds=_BACKOFF_RESET_AFTER_SECONDS, proactive_reconnect_after_seconds=_PROACTIVE_RECONNECT_AFTER_SECONDS, parse_message=_json.loads, market_endpoint=_WS_MARKET, stale_symbols=self._stale_symbols, maybe_backfill_after_disconnect=self._maybe_backfill_after_disconnect, compute_disconnect_delay=compute_disconnect_delay, connection_exceptions=(ws_exceptions.ConnectionClosed, ws_exceptions.InvalidStatus, ConnectionError, TimeoutError, OSError))

    def _handle_ws_response(self, msg: JsonDict, endpoint: str) -> None:
        if msg.get('error'):
            self._subscription_errors[endpoint] = msg['error']
            LOG.error('ws subscription error from Binance | endpoint=%s error=%s', endpoint, msg['error'])
            return
                ...
                if isinstance(rate_limits, list):
                    for limit in rate_limits:
                        ...
                        if limit_type and count is not None and (limit_val is not None):
                            ...
                            if usage_pct > 95:
                                LOG.error('ws rate limit high usage | endpoint=%s type=%s interval=%s count=%d limit=%d usage=%.1f%%', endpoint, limit_type, interval, count, limit_val, usage_pct)
                            elif usage_pct > 80:
                                LOG.info('ws rate limit elevated usage | endpoint=%s type=%s interval=%s count=%d limit=%d usage=%.1f%%', endpoint, limit_type, interval, count, limit_val, usage_pct)
        ...
        if isinstance(result, list):
            ...
            actual = len(result)
            if expected != actual:
                LOG.debug('ws subscriptions mismatch | endpoint=%s expected=%d actual=%d', endpoint, expected, actual)
            else:
    ...
    async def _dispatch_event(self, data: JsonDict) -> None:
        ...
        if event_time is not None:
            try:
                ...
                if symbol and event_type in _LATENCY_WARNING_EVENTS and (self._last_event_lag_ms >= _HIGH_EVENT_LATENCY_MS):
                    now = time.monotonic()
                    ...
                    if now - last_warn >= _LATENCY_WARNING_INTERVAL_SECONDS:
                        ...
                        LOG.debug('ws high latency | lag_ms=%.1f symbol=%s event=%s', self._last_event_lag_ms, symbol, event_type)
            except (TypeError, ValueError):
                pass
        if event_type == 'kline':
            if symbol:
    ...
    async def _process_message_internal(self, msg: JsonDict) -> None:
        ...
        if isinstance(data, dict):
            order = data.get('o')
            str(data.get('s') or (order.get('s') if isinstance(order, dict) else None) or str(stream).split('@', 1)[0] or 'unknown').upper()
        ...
        if isinstance(data, dict):
            ...
            if len(self._stream_latency_ms[stream]) >= 5:
                ...
                if avg_latency > 5000 and stream not in self._slow_streams:
                    ...
                    LOG.error('slow stream detected | stream=%s avg_latency_ms=%.1f', stream, avg_latency)
    ...
    async def _handle_kline(self, symbol: str, data: JsonDict) -> None:
        ...
        async with self._data_lock:
            ...
            if interval not in self._klines[symbol]:
                self._klines[symbol][interval] = collections.deque(maxlen=self._cfg.kline_cache_size)
        ...
        if self._event_bus is not None:
            self._event_bus.publish_nowait(KlineCloseEvent(symbol=symbol, interval=interval, close_ts=close_ts_ms))
