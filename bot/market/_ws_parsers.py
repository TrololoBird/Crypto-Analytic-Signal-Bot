...
def _ws_kline_to_row(k: JsonDict) -> JsonDict:
    return {'time': datetime.fromtimestamp(int(k['t']) / 1000.0, tz=UTC), 'open': float(k['o']), 'high': float(k['h']), 'low': float(k['l']), 'close': float(k['c']), 'volume': float(k['v']), 'close_time': datetime.fromtimestamp(int(k['T']) / 1000.0, tz=UTC), 'quote_volume': float(k['q']), 'num_trades': int(k['n']), 'taker_buy_base_volume': float(k['V']), 'taker_buy_quote_volume': float(k['Q'])}

def _clamp(value: float) -> float:
    return max(-1.0, min(1.0, value))

def depth_imbalance_from_book(*, bid_qty: float | None, ask_qty: float | None, delta_ratio: float | None) -> float | None:
...
def microprice_bias_from_book(*, bid: float | None, ask: float | None, bid_qty: float | None=None, ask_qty: float | None=None, delta_ratio: float | None) -> float | None:
...
def _update_depth_wall_pressure(manager: Any, symbol: str, bids: tuple[tuple[float, float], ...], asks: tuple[tuple[float, float], ...], now: float) -> None:
    ...
    if total > 0.0:
        manager._depth_wall_pressure[symbol] = round(max(-1.0, min(1.0, (bid_wall - ask_wall) / total)), 4)
    else:
        manager._depth_wall_pressure.pop(symbol, None)
...
def handle_ticker(manager: Any, symbol: str, data: JsonDict) -> None:
    ...
    if should_throttle_ticker_update(manager, symbol):
        return
    try:
        manager._ticker_cache[symbol] = {'symbol': symbol, 'last_price': float(data.get('c') or 0.0), 'quote_volume': float(data.get('q') or 0.0), 'price_change_percent': float(data.get('P') or 0.0), 'price_change': float(data.get('p') or 0.0), 'open_price': float(data.get('o') or 0.0), 'high_price': float(data.get('h') or 0.0), 'low_price': float(data.get('l') or 0.0), 'trade_count': int(float(data.get('n') or 0))}
...
def handle_mini_ticker(manager: Any, symbol: str, data: JsonDict) -> None:
    ...
    try:
        ...
        manager._ticker_cache[symbol] = {'symbol': symbol, 'last_price': close_price, 'quote_volume': float(data.get('q') or 0.0), 'price_change_percent': price_change_pct, 'price_change': close_price - open_price, 'open_price': open_price, 'high_price': float(data.get('h') or 0.0), 'low_price': float(data.get('l') or 0.0)}
        manager._ticker_cache_ts = time.monotonic()
    except (TypeError, ValueError):
        return
...
def handle_mark_price(manager: Any, symbol: str, data: JsonDict) -> None:
    ...
    try:
        ...
        if mark_price <= 0.0:
            return
        now = time.monotonic()
        manager._mark_price_cache[symbol] = {'symbol': symbol, 'mark_price': mark_price, 'index_price': float(data.get('i') or 0.0), 'funding_rate': funding_rate, 'next_funding_time_ms': int(data.get('T') or 0), 'updated_at': now}
...
async def handle_book_ticker(manager: Any, symbol: str, data: JsonDict) -> None:
    ...
    if manager._event_bus is not None:
        manager._event_bus.publish_nowait(BookTickerEvent(symbol=symbol, bid=bid, ask=ask, event_ts_ms=event_ts_ms))
...
async def handle_agg_trade(manager: Any, symbol: str, data: JsonDict) -> None:
    if manager._symbols and symbol not in manager._symbols:
        return
    try:
        trade = AggTrade(symbol=symbol, trade_id=int(data['a']), price=float(data['p']), quantity=float(data['q']), trade_time_ms=int(data['T']), is_buyer_maker=bool(data['m']))
    except (KeyError, TypeError, ValueError):
        return
