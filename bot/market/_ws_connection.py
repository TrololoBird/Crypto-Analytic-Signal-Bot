...
def compute_disconnect_delay(manager: Any, *, endpoint: str, url: str, exc: Exception, elapsed: float, delay: float) -> float:
    ...
    LOG.log(level, 'ws disconnected | endpoint=%s url=%s reason=%s close_detail=%s uptime=%.1fs retry_in=%.1fs streak=%d', endpoint, url, log_reason, close_detail, elapsed, next_delay, manager._short_lived_streak)
    return next_delay
...
async def evaluate_endpoint_health(manager: Any, ws: Any, endpoint: str) -> bool:
    ...
    """
    max_silence = float(getattr(manager._cfg, 'silence_timeout_seconds', getattr(manager._cfg, 'health_check_silence_seconds', 60.0)))
    ...
    if last_message_ts > 0.0:
        ...
        if last_message_age > max_silence:
            LOG.info('ws endpoint silence exceeded | endpoint=%s age=%.1fs max=%.1fs', endpoint, last_message_age, max_silence)
            await ws.close()
            return True
    ...
    if endpoint == 'market':
        ...
        if stale_streams:
            ...
            LOG.info('ws stale kline data | endpoint=%s streams=%d sample=%s - backfilling (not reconnecting)', endpoint, len(stale_streams), preview)
        ...
        return False
    if endpoint == 'public':
        fresh_books = sum((1 for sym, ts in manager._book_update_times.items() if sym in manager._symbols and time.monotonic() - ts <= manager._cfg.market_ticker_freshness_seconds))
...
async def monitor_connection_silence(manager: Any, ws: Any, endpoint: str) -> bool:
    ...
    if endpoint == 'public' and any(('@bookticker' in str(stream).lower() or '@depth' in str(stream).lower() for stream in streams)):
    ...
    if silence > silence_limit and streams:
        LOG.info('ws health: no message for %.0fs with %d streams - forcing reconnect | endpoint=%s', silence, len(streams), endpoint)
        await ws.close()
        return True
    return False
...
def apply_connected_state(manager: Any, *, endpoint: str, ws: Any, url: str) -> None:
    ...
    LOG.info('ws connected | endpoint=%s url=%s streams=%d connect_count=%d endpoint_connect_count=%d', endpoint, url, len(manager._intended_streams_by_endpoint.get(endpoint, set())), manager._connect_count, manager._connect_counts[endpoint])
...
async def run_stream_session(manager: Any, *, endpoint: str, url: str, connect_start: float, backoff_reset_after_seconds: float, proactive_reconnect_after_seconds: float, parse_message: Any) -> tuple[bool, bool]:
    ...
    connect_kwargs: dict[str, Any] = websockets_connect_kwargs(proxy_url=proxy_url, trust_env=trust_env)
    ws = await asyncio.wait_for(websockets.connect(url, ping_interval=_WS_PING_INTERVAL_SECONDS, ping_timeout=_WS_PING_TIMEOUT_SECONDS, close_timeout=_WS_CLOSE_TIMEOUT_SECONDS, **connect_kwargs), timeout=_WS_CONNECT_TIMEOUT_SECONDS)
    ...
    async with ws:
        ...
        if stream_count > 120:
            LOG.info('high stream count | endpoint=%s streams=%d shortlist=%d', endpoint, stream_count, len(manager._symbols))
        health_task = asyncio.create_task(manager._health_monitor(ws, endpoint), name=f'ws_manager_health:{endpoint}')
        try:
            async for raw in ws:
    ...
    if reconnect_reason == '24h_proactive':
        ...
        LOG.info('ws proactive reconnect | endpoint=%s uptime=%.1fh', endpoint, (time.monotonic() - connect_start) / 3600)
...
async def run_endpoint_loop(manager: Any, *, endpoint: str, backoff_reset_after_seconds: float, proactive_reconnect_after_seconds: float, parse_message: Any, market_endpoint: str, stale_symbols: Any, maybe_backfill_after_disconnect: Any, compute_disconnect_delay: Any, connection_exceptions: tuple[type[BaseException], ...]) -> None:
    ...
    while manager._running:
        ...
        backoff_reset = False
        try:
            LOG.info('ws connecting | endpoint=%s url=%s streams=%d', endpoint, url, len(manager._intended_streams_by_endpoint.get(endpoint, set())))
            backoff_reset, proactive_reconnect = await run_stream_session(manager, endpoint=endpoint, url=url, connect_start=connect_start, backoff_reset_after_seconds=backoff_reset_after_seconds, proactive_reconnect_after_seconds=proactive_reconnect_after_seconds, parse_message=parse_message)
            retry_streak = 0
            if backoff_reset:
                delay = 1.0
            if proactive_reconnect:
                ...
                if endpoint == market_endpoint:
                    stale = stale_symbols()
                    await maybe_backfill_after_disconnect(elapsed=time.monotonic() - connect_start, stale_symbols=stale)
                continue
            ...
            if retry_streak <= 3:
                ...
                delay = compute_disconnect_delay(manager, endpoint=endpoint, url=url, exc=exc, elapsed=elapsed, delay=delay)
            try:
            ...
            LOG.exception('ws unexpected error during connection | endpoint=%s (%s)', endpoint, type(exc).__name__)
            clear_endpoint_connection_state(manager, endpoint)
            if not manager._running:
                return
            retry_streak += 1
