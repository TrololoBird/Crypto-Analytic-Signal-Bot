...
# Win reasons
WIN_REASONS = {"tp1_hit", "tp2_hit", "trailing_stop"}
NON_TRADE_RESULTS = {
    "expired_pending",
    "unactivated_close",
    "superseded",
...
def _is_win(record: dict[str, Any]) -> bool:
    if isinstance(record.get("was_profitable"), bool):
        return bool(record["was_profitable"])
    pnl = _safe_float(record.get("pnl_r_multiple"))
    if pnl is not None:
        return pnl > 0.0
...
def _is_trade_record(record: dict[str, Any]) -> bool:
    result = str(record.get("result") or record.get("close_reason") or "").strip()
    if result in NON_TRADE_RESULTS:
        return False
    if record.get("entry_price") is None or record.get("exit_price") is None:
        return False
    return _safe_float(record.get("pnl_r_multiple")) is not None
...
def _find_threshold(
    ...
    for rec in records:
        ...
        if value is not None:
            values.append((value, _is_win(rec)))
    ...
    for i in range(n_bins):
        ...
        bucket = [
            (v, w)
            for v, w in values
            if (bin_lo <= v <= bin_hi if i == n_bins - 1 else bin_lo <= v < bin_hi)
        ]
        if len(bucket) < 5:
            continue
...
def compute_optimal_thresholds(data_path: str | Path) -> dict[str, Any]:
    ...
    trade_records = [record for record in records if _is_trade_record(record)]
    trade_sample_count = len(trade_records)

    if trade_sample_count < MIN_OUTCOMES:
        LOG.info(
            "autotuner: insufficient trade data (%d < %d required, raw=%d)",
            trade_sample_count,
            MIN_OUTCOMES,
            sample_count,
        )
    ...
    LOG.info(
        "autotuner: processing %d trade outcome records (raw=%d)",
        trade_sample_count,
        sample_count,
    )

    # --- Threshold recommendations ---
    min_score = _find_threshold(trade_records, "score") or _find_threshold(
        trade_records, "base_score"
    )
    min_rr = _find_threshold(trade_records, "risk_reward")

    result: dict[str, Any] = {
        "sample_count": sample_count,
        "trade_sample_count": trade_sample_count,
        "generated_at": datetime.now(UTC).isoformat(),
        "setup_win_rates": _setup_win_rates(trade_records),
    }
    ...
    try:
        ...
        lines = [
            ...
            f"# trade_sample_count = {trade_sample_count}",
            "",
            "[filters]",
        ]
        if min_score is not None:
