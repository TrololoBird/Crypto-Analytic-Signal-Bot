...
def _vwap_expr(df: pl.DataFrame) -> pl.Expr:
    """Return a VWAP expression, reset per UTC session when timestamps exist."""
    ...
    if time_column is not None:
        ...
        session_key = pl.col(time_column).dt.date()
        cum_pv = pv.cum_sum().over(session_key)
        cum_vol = pl.col("volume").cum_sum().over(session_key)
        return (cum_pv / cum_vol).fill_nan(pl.col("close"))

    return (pv.cum_sum() / pl.col("volume").cum_sum()).fill_nan(pl.col("close"))
...
def _prepare_frame(df: pl.DataFrame) -> pl.DataFrame:
    ...
    work = work.with_columns(
        [
            _vwap_expr(work).alias("vwap"),
        ]
    )

    # Calculate VWAP standard deviation with session-awareness
    time_column = next(
        (
            column
            for column in ("close_time", "time", "open_time")
    ...
    price_dev_sq = (pl.col("close") - pl.col("vwap")) ** 2
    if time_column is not None:
        session_key = pl.col(time_column).dt.date()
        # Anchored standard deviation since session start
        cum_dev_sq = price_dev_sq.cum_sum().over(session_key)
        # Sequence count within session
        session_n = pl.lit(1).cum_sum().over(session_key)
        vwap_std_expr = (cum_dev_sq / session_n).sqrt()
    else:
        denom = pl.lit(1).cum_sum()
        vwap_std_expr = (price_dev_sq.cum_sum() / denom).sqrt()

    work = work.with_columns(
        [
            vwap_std_expr.alias("vwap_std"),
        ]
    )
