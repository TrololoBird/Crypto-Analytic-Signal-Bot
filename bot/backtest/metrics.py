...
class BacktestResult:
    ...
    def from_frames(cls, trades: pl.DataFrame, equity_curve: pl.DataFrame) -> "BacktestResult":
        ...
        activation_rate = _as_float(trades.get_column("activated").mean(), 0.0) if "activated" in trades.columns else 0.0
        tp1_rate = _as_float(trades.get_column("tp1_hit").mean(), 0.0) if "tp1_hit" in trades.columns else 0.0
        tp2_rate = _as_float((trades.get_column("status") == "tp2").cast(pl.Float64).mean(), 0.0) if "status" in trades.columns else 0.0
        sl_rate = _as_float((trades.get_column("status") == "sl").cast(pl.Float64).mean(), 0.0) if "status" in trades.columns else 0.0
        expired_rate = _as_float((trades.get_column("status") == "expired").cast(pl.Float64).mean(), 0.0) if "status" in trades.columns else 0.0
        avg_resolution_bars = _as_float(trades.get_column("resolution_bars").mean(), 0.0) if "resolution_bars" in trades.columns else 0.0
        ...
        if "setup_id" in trades.columns and "status" in trades.columns and trades.height > 0:
            ...
            for row in trades.select(["setup_id", "status"]).to_dicts():
                ...
                bucket = setup_breakdown.setdefault(setup_id, {"count": 0, "tp2": 0, "tp1": 0, "sl": 0, "expired": 0})
