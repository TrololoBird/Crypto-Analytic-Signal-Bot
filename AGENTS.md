# AGENTS.md — Project context for Codex agent
...
0. **SCOPE LOCK — `hunt/` ONLY.** This agent operates exclusively inside `hunt/`. Never read, write, search, or modify files outside `hunt/` except `.env`. Cross-package imports (`engine.*`, `bot.*`) are forbidden to reference or depend on. The main bot (`bot/`, `engine/`, `main.py`, `scripts/`, etc.) is out of bounds — act as if it does not exist.
1. No auto-trading logic
2. No authenticated Binance endpoints
3. No new test files or mock harnesses
4. All edits must pass: `python -m py_compile $(find hunt -name "*.py")`
