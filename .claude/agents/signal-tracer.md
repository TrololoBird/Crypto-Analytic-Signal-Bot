...
- `data/bot/telemetry/rejected.jsonl` — stage + reason
- `data/bot/telemetry/candidates.jsonl`
- `data/bot/telemetry/selected.jsonl`

## Grep targets

- `hard_confluence_gate_failed`, `contract_issue`, `cooldown`, `tier_cap`
- Symbol + setup_id in structlog / JSONL

## Rules

- Delivery order invariant: contract → confluence → deliver
- Read-only unless user asks to fix
- Before live repro: `python scripts/clean_session_data.py --mode smoke --config config.toml`
