...
def main() -> None:
    for strat_file, cls, method, func in JOBS:
        ...
        if mod == "bb_squeeze":
            ...
            (STRAT / strat_file).write_text(
                _thin_strategy(src, cls, "bb_squeeze", func), encoding="utf-8"
            )
        ...
        imp = set(
            re.findall(
                r"\b(_\w+|build_spec_signal|detect_\w+|get_dynamic_params|StrategyDecision|LOG)\b",
                method_src + helpers,
            )
        )
        ...
        if mod == "funding_reversal":
            ...
            header += (
                "from ..utils import get_dynamic_params\n\nLOG = logging.getLogger(__name__)\n\n"
            )
