...
def analyze_null_fields(run_dir: Path) -> dict:
    ...
    try:
        ...
        all_null = [
            ...
            if null_stats[f]["total_count"] > 0
            and null_stats[f]["null_count"] == null_stats[f]["total_count"]
        ]
        if all_null:
...
def print_audit_report(
    ...
    if "error" in funnel_stats:
        ...
        if funnel_stats.get("total_raw_hits", 0) > 0:
            conversion = (
                funnel_stats.get("total_candidates", 0)
                / funnel_stats.get("total_raw_hits", 1)
                * 100
            )
    ...
    if "error" in cycle_stats:
        ...
        if cycle_stats.get("total_detector_runs", 0) > 0:
            hit_rate = (
                cycle_stats.get("total_candidates", 0)
                / cycle_stats.get("total_detector_runs", 1)
                * 100
            )
    ...
    if "error" in decision_stats:
        ...
        for (setup, stage, reason), count in decision_stats.get(
            "rejects_by_setup_reason", Counter()
        ).most_common(15):
    ...
    if total_rejections > 0:
        no_raw_hit_pct = (
            rejection_stats.get("by_reason", Counter()).get("no_raw_hit", 0)
            / total_rejections
            * 100
        )
