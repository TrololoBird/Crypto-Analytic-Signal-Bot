...
| `live_check_pipeline.py` | Live-проверка полной воронки кандидатов без отправки Telegram-сообщений; без явных символов выбирает top-volume USDT contracts, если run-id недоступен. | active | `--symbols`, `--symbols-from-run`, `--limit`, `--concurrency`, `--no-warm-context`, `--include-basis` | `python -m scripts.live_check_pipeline --limit 12 --concurrency 2` |
