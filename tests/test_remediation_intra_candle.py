...
async def test_intra_candle_path_emits_cycle_log() -> None:
    ...
    bot._select_and_deliver_for_symbol = AsyncMock(
...
async def test_intra_candle_uses_event_timestamp_when_available() -> None:
    ...
    await bot._on_book_ticker(
        BookTickerEvent(symbol="BTCUSDT", bid=100.0, ask=100.1, event_ts_ms=event_ts_ms)
    )
    ...
    assert ws_enrichments["spread_bps"] == pytest.approx(
        ((100.1 - 100.0) / 100.05) * 10000.0
    )


@pytest.mark.asyncio
async def test_intra_candle_skips_small_book_move_when_min_move_bps_configured() -> (
    None
):
    ...
    await bot._on_book_ticker(
        BookTickerEvent(symbol="BTCUSDT", bid=100.004, ask=100.104)
    )
    for task in list(bot._background_tasks):
        await asyncio.gather(task)
