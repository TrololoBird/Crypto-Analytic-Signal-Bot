"""Live gate: strategy catalog wiring matches STRATEGY_CLASSES (no Binance call)."""
...
@pytest.mark.live
def test_strategy_catalog_registered() -> None:
    errors = verify_strategy_wiring(STRATEGY_CLASSES)
    assert not errors, "catalog wiring errors:\n" + "\n".join(errors)
    assert len(STRATEGY_CLASSES) == len(CATALOG_SETUP_IDS) == len(CATALOG_ENTRIES)
    assert len(STRATEGY_CLASSES) >= 28
...
def test_r_class_watch_only_policy() -> None:
    assert frozenset({"price_velocity"}) == R_CLASS_SETUP_IDS
    ...
    assert r_class_blocks_action("price_velocity", settings)
