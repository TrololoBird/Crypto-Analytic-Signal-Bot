"""ADX / VWAP / supertrend / OBV penalties for hunt setup scoring.

Single source for watch._dump_analysis / _long_analysis and logic_verify.
Phase-aware: trend-continuation filters go soft when the trend *is* the setup
(exhaustion fade short, initial pump long).
"""
...
from hunt_watch.param_store import effective_hunt_params, filter_thresholds

def _vwap_extreme_atr(symbol: str = "") -> float:
    flt = filter_thresholds(symbol)
    return float(flt.get("vwap_extreme_atr", 2.25))
ADX_TREND_MIN = 30.0  # meme perp consensus (research batch 1)
ADX_TREND_BLOCK = 40.0  # fallback; runtime uses active_params().adx_trend_block
DI_DOMINANCE = 1.25
...
def directional_filters(
    ...
    *,
    ...
    if direction == "short":
        ...
        if st_dir == 1:
            delta -= 8.0
            triggers.append("supertrend_1h_up")
        ...
        if st_dir == -1:
            delta -= 8.0
            triggers.append("supertrend_1h_down")
        if vdev is not None and float(vdev) >= vwap_extreme:
