...
def _structural_targets(
    ...
    *,
    ...
    zone(s), nearest first, searched across EVERY configured timeframe, not just the
    one the entry zone itself was found on. A support bounce found on 1h routinely
    has to travel through a zone that only shows up on 4h/15m/1d — restricting the
    search to the entry's own TF made a real, well-touched target on a coarser or
    finer scale invisible, and the whole trade abstained for lack of a target that
    was right there on another timeframe (course example: REZ's 1h add-long zone
    0.003063-0.00307367 targeting the 0.0031-0.0034 box that only resolves on 4h/15m).
    ...
    for tf, raw in ohlcv_by_tf.items():
        ...
        if lookback is None or not raw:
            continue
    ...
    if direction == "long":
        zone_edges = sorted((z["lo"] for z in pool if z["lo"] > entry))
    else:
        zone_edges = sorted((z["hi"] for z in pool if z["hi"] < entry), reverse=True)
...
def _base_summary(
    *,
    ...
    entry_lo, entry_hi = _entry_band(entry)
    if direction == "wait":
        geo: dict[str, Any] = {}
    else:
        geo = _geometry_from_zone(direction=direction, entry=entry, ohlcv_by_tf=ohlcv_by_tf, cfg=cfg, swing_levels=swing_levels)
        if geo is None:
...
def _pp_candidate(
    *,
    ...
    if pp.get("pp_true_long") or pp.get("pp_early_long"):
        level = pp.get("pp_true_long_level") or pp.get("pp_early_long_level")
        long_swing = _extract_swing_levels(struct_by_tier, direction="long", entry=level)
        summary = _base_summary(
            direction="long", entry=level, zone={"hi": zone["hi"], "lo": min(zone["lo"], level)},
            setup_kind="pp_break", tf_tier=tier_name, tf=tf, catalyst_level=level, poc_info={},
            ohlcv_by_tf=ohlcv_by_tf, cfg=cfg, swing_levels=long_swing,
        )
        if summary is None:
            return None
        summary["gates_failed"] = [] if pp.get("pp_true_long") else ["ranний_pp_unconfirmed"]
        summary["geometry_confidence"] = 0.7 if pp.get("pp_true_long") else 0.5
        result = _apply_confluence(
            summary, ohlcv=ohlcv, btc_d_change_24h=btc_d_change_24h, total3_change_24h=total3_change_24h,
            cfg=cfg, htf_bias=htf_bias, struct_by_tier=struct_by_tier,
        )
        if result is not None:
            result["invalidation"] = build_invalidation(
                direction="long", entry_lo=result.get("entry_lo", level * 0.998),
                entry_hi=result.get("entry_hi", level * 1.002),
    ...
    if pp.get("pp_true_short") or pp.get("pp_early_short"):
        level = pp.get("pp_true_short_level") or pp.get("pp_early_short_level")
        short_swing = _extract_swing_levels(struct_by_tier, direction="short", entry=level)
        summary = _base_summary(
            direction="short", entry=level, zone={"hi": max(zone["hi"], level), "lo": zone["lo"]},
            setup_kind="pp_break", tf_tier=tier_name, tf=tf, catalyst_level=level, poc_info={},
            ohlcv_by_tf=ohlcv_by_tf, cfg=cfg, swing_levels=short_swing,
        )
        if summary is None:
            return None
        summary["gates_failed"] = [] if pp.get("pp_true_short") else ["ранний_pp_unconfirmed"]
        summary["geometry_confidence"] = 0.7 if pp.get("pp_true_short") else 0.5
        result = _apply_confluence(
            summary, ohlcv=ohlcv, btc_d_change_24h=btc_d_change_24h, total3_change_24h=total3_change_24h,
            cfg=cfg, htf_bias=htf_bias, struct_by_tier=struct_by_tier,
        )
        if result is not None:
            result["invalidation"] = build_invalidation(
                direction="short", entry_lo=result.get("entry_lo", level * 0.998),
                entry_hi=result.get("entry_hi", level * 1.002),
                stop=result.get("stop", 0), catalyst_level=level, zone=zone,
