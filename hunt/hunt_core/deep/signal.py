"""Module 1 Deep — signal helpers: BTC alignment, MTF, liquidity, order flow, POC scenarios."""
...
def _setup_strength(setup: dict[str, Any], *, direction: str) -> float:
    """0–100 strength for direction pick — P(win) first, fusion magnitude fallback."""
    ...
    for key in ("fusion_score", "magnitude", f"{direction}_fuel", "dump_fuel", "long_fuel"):
        try:
            ...
            if raw is not None:
                val = float(raw)
                # Fuel scores are already 0–100-ish; magnitudes may be z-scale.
...
def fmt_flow_qty(value: float | None) -> str:
    """Compact signed flow label — re-export from shared facts."""
    from hunt_core.shared.facts.order_flow import fmt_flow_qty as _fmt
