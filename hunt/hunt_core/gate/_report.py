...
from hunt_core.gate._filters import hard_filter_blocks as _hard_filter_blocks
from hunt_core.gate._freshness import classify_delivery_tier
from hunt_core.gate._lifecycle import lifecycle_dict as _lifecycle_dict
from hunt_core.gate._lifecycle_gates import collect_lifecycle_blockers
from hunt_core.gate._registry import pipeline_pre_blockers as _pipeline_pre_blockers
...
from hunt_core.gate._types import GateResult, REPORT_BLOCK_PRIORITY
from hunt_core.gate.policy import direction_block_reason, run_declarative_delivery_gates
from hunt_core.params.store import delivery_thresholds, effective_hunt_params
def blocks_premature_exhaustion_short(*_args, **_kwargs) -> tuple[bool, str]:
    # PRE/MID phase is now decided by the fusion engine; this legacy guard is a no-op.
    return False, ""
...
def collect_report_blockers(
    ...
    *,
    ...
    from hunt_core.gate._phase_matrix import phase_matrix_gate
    ...
    if direction == "long":
        ...
        from hunt_core.gate._delivery_helpers import long_resistance_chase_veto
...
def evaluate_alert_gate(
    ...
    *,
    ...
    if blockers:
        return blockers[0]
    if row is not None:
        from hunt_core.gate._maps import map_opposing_bias_veto

        veto, reason = map_opposing_bias_veto(row, direction=direction)
        if veto and reason:
            return GateResult(False, reason, f"Map liquidity veto: {reason}")
