...
def _record_deep_outcome(signal: Signal, row: dict[str, Any], *, event: str) -> None:
    try:
        ...
        _LOG.exception("deep_outcome_ledger_failed", symbol=signal.symbol)
...
def _register_tracker(signal: Signal, row: dict[str, Any]) -> None:
    try:
        ...
        from hunt_core.track.tracker import load_state, register_signal_open, save_state
        ...
        _LOG.exception("signal_tracker_register_failed", symbol=signal.symbol)
