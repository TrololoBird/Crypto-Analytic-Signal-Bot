...
| `watchFundingRates` | varies | `hunt_ccxt_funding_cross` | Secondaries only when `HUNT_CROSS_WS=1` |
...
```bash
cd hunt
PYTHONPATH=. python -m hunt_core._dev.check_ccxt
PYTHONPATH=. python -m hunt_core._dev.ccxt_plane_smoke BTCUSDT ETHUSDT --ws-seconds 3
PYTHONPATH=. python -m hunt_core._dev.check_logic
```
